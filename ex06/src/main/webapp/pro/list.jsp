...
<script id="temp_product" type="text/x-handlebars-template">
	<table class="table">
		...
		<tr>
			...
			<td>
				<input value="{{price}}" class="price" oninput="isNumber(this)">
				<span class="fprice">{{fprice}}</span>
			</td>
			<td>{{fdate}}</td>
			<td><button class="btn btn-danger btn-sm" code="{{code}}">삭제</td>
...
<script>
	...
	//가격이 바뀔경우
	$("#div_product").on("keydown", ".price", function(e){
		const fprice=$(this).parent().find(".fprice");
		const price=$(this).val();
		fprice.html(price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))
	});
		...
		if(confirm("상품정보를 수정하실래요?")){
			$.ajax({
				...
				success:function(){
					alert("수정완료!");					﻿
					//getTotal();
					//수정은 수정페이지 그대로 두는 것으로 보기에 좋을 것 같아서 1페이지 안감
					//gettotal() 수정 안함
